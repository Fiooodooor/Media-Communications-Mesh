...
YELLOW="33m"
BLUE="34m"
PromptTBlue='\e[38;05;45m'
PromptHBlue='\e[38;05;33m'
ErrorHRed='\e[38;05;1m'
SuccessHGreen='\e[38;05;2m'
WarningHYellow='\e[38;05;3m'
WarningHPurple='\e[38;05;61m'
EndCl='\e[m'
...
function message() {
    ...
    if [ -z "${DISABLE_COLOR_PRINT}" ]; then
        FOOTER='\e[0m'
        case "${type}" in
          ERROR)        HEADER="${BOLD}${RED}${type}:  ${REGULAR}${RED}" ;;
          WARN|WARNING) HEADER="${BOLD}${BLUE}${type}: ${REGULAR}${YELLOW}" ;;
          SUCC|SUCCESS) HEADER="${BOLD}${BLUE}${type}: ${REGULAR}${GREEN}" ;;
          INFO|*)       HEADER="${BOLD}${BLUE}${type}: ${REGULAR}${BLUE}" ;;
        esac
    fi
...
function warning() {
    # message "${WarningHYellow}WARN${PromptTBlue}" "$*${EndCl}"
    message "WARN" "$*"
}

function log_info()    { message "INFO" "$*"; }
function log_success() { message "SUCCESS" "$*"; }
function log_warning() { message "WARNING" "$*"; }
